...
fn tag_pages_are_filtered_and_sorted() {
    ...
    assert!(pos_new < pos_old, "rust tag page must be sorted by date desc then title");

    let bad_tag_path = tmp.path().join(OUTPUT_DIR).join(TAGS_DIR).join("bad tag.html");
...
fn asset_prefixes_match_depth() {
    ...
    runner
        .run(&rel_markdown_path(), |rel_path| {
            ...
            let expected_css = format!(r#"href="{}style.css""#, prefix);
            let expected_index = format!(r#"href="{}index.html""#, prefix);

            prop_assert!(html.contains(&expected_css));
            prop_assert!(html.contains(&expected_index));
            Ok(())
        })
        .unwrap();
}
